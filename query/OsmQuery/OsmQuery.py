...
def get_queries_from_tags(config_dir, config_name, out_format, timeout, tags, conditions):
    ...
    possible_values["out_format"] = [out_format]
    possible_values["timeout"] = [timeout]
    ...
    for tag, tag_values in tags.items():
        ...
        for tag_value, tag_value_conditions in tag_values.items():
            ...
            for condition_name, condition_value in tag_value_conditions.items():
                if condition_name not in conditions.keys():
                    continue
                condition_evaluator = conditions[condition_name]
...
def execute_query(endpoint, query, output_dir, output_ext):
    ...
    if os.path.exists(fname):
        print(f"\tFile {fname} already exists, skipping query {query_name}")
        return
    ...
    if resp.status_code != 200:
        ...
        print(f"\t{query_name} returned code {resp.status_code}")
        fname = os.path.join(output_dir, f"{query_name}.{output_ext}")
...
def execute_queries(endpoint, queries, output_dir, output_ext):
    if not queries:
        return
    for q in queries:
        execute_query(endpoint, q, output_dir, output_ext)
...
def main():
    ...
    for config_name in config_names:
        ...
        query_texts = get_queries_from_tags(config_dir, config_name, out_format, timeout, tags, {"zoom" : lambda x: int(x) <= zoom})
        execute_queries(endpoint, query_texts, output_dir, out_format)
